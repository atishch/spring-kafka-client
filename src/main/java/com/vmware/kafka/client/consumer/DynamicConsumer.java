...
import com.vmware.kafka.client.KafkaClientConstants;
import com.vmware.kafka.client.ProcessDelegator;
import com.vmware.kafka.client.offset.KafkaDefaultOffsetService;
import com.vmware.kafka.client.offset.OffsetService;
import com.vmware.kafka.client.utility.KafkaUtils;
...
public class DynamicConsumer<K extends Serializable, V extends Serializable> implements Runnable {
	...
	public DynamicConsumer(ConsumerConfigurer<K,V> config){
		...
		this.offsetService = new KafkaDefaultOffsetService(consumer);
		init(null);
	}
	
	public DynamicConsumer(String clientId,List<String> topics,Properties consumerProps,ProcessDelegator<K,V> processDelegator) {
